...
image versus a distractor). I used *mixed-effects* models to estimate a
...
distractors at 250 ms, 300 ms, etc.—were fit using an orthogonal
cubic polynomial function of time [a now-conventional approach; see
...
centered around 0. The curves adjusted up and down to their correct
position by the model's intercept term. This is why in these models, the
intercept is not the value at some time 0 but instead the average value
of the fitted growth curve. For these word recognition models, the
...
```{r effects2, fig.cap = "(ref:effects2)", echo = FALSE, out.width = "100%", fig.height=4, fig.width=8}
...
```{r pairwise-effects, fig.cap = "(ref:pairwise-effects)", fig.show = 'hold', echo = FALSE, out.width = "100%", fig.height = c(4), fig.width = c(8)}
clean_names <- . %>% 
