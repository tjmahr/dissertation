...
```{r curve-features, fig.cap = "(ref:curve-features)", echo = FALSE, out.width = "80%", fig.height = 5, fig.width = 5}
dummy_data <- d_m %>% 
...
lpred %>% 
  ...
  ggplot() + 
    ...
    geom_text(
      aes(color = "Age 5", label = "Age 5"), 
      x = 1390, y = .83, family = "Lato") +
    geom_text(
      aes(color = "Age 3", label = "Age 3"), 
      x = 1380, y = .46, family = "Lato") +
    expand_limits(y = .85) +
    guides(color = "none") +
    labs(
...
ggplot(by_draw %>% mutate(Study = convert_study_to_age(Study))) +
  ...
  geom_point() + 
  geom_text(aes(label = Study), data = range_labels, family = "Lato") +
