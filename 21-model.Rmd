...
prior_sd <- set_prior("student_t(3, 0, 5)", class = "sd")
priors <- c(prior_intercept, prior_b, prior_ot1, prior_cor, prior_sd)

test_m <- brm(test_f, data = test_data, prior = priors, chains = 4, cores = 4, 
              control = list(adapt_delta = .95))
readr::write_rds(test_m, "tp1_mp_2.rds.gz")
# test_m <- readr::read_rds("tp1_mp.rds.gz")
...
ggplot(pl) + 
  ...
  # geom_line(aes(group = interaction(.draw, Condition)), alpha = .1) + 
  stat_summary() +
  stat_summary(aes(y = Target / Trials), data = test_data)
