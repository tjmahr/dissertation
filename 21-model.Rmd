...
ggplot(d_test) + 
  ...
  labs(
    ...
    caption = 
      "Intervals: Observed proportions ± SE. Lines: 100 posterior fits") + 
  guides(color = FALSE)
...
curves for each child x condition x condition. To measure children's
...
curves. This difference in peak values conveys the _condition advantage_ for a child. 
The figure below visualizes the condition advantages.
...
peaks %>% 
  ...
  summarise_at(vars(nonsense:diff), mean) %>% 
    ...
    labs(
      ...
      x = NULL,
      y = "Nonword peak - real word peak",
      caption = "Open points: Participant posterior means. Closed: Mean of means.") + 
    theme(
...
curve_diffs <- all_fits %>% 
  ...
  select(Study, .draw, ResearchID, Time, nonsense_prop, real_prop, `real - nonsense`)


some_fits <- all_fits %>% 
  tjmisc::sample_n_of(50, .draw)
...
p_diffs <- ggplot(some_curve_diffs) + 
  ...
  geom_text(
    ...
    hjust = 0,
    size = 3, color = constants$col_off_black) + 
  facet_wrap("Study") + 
  expand_limits(y = c(-.1, .2)) +
  labs(
    x = constants$x_time,
    y = "Difference",
    caption = "Lines: 50 posterior means. Points: Empirical means.") +
  theme(
...
In chapter X, I found that peak looking probability at age 3
positively correlated with age 5 vocabulary. Above, I showed that the
growth curve peaks differed between the two conditions, especially at
