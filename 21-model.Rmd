...
# test_m_all <- readr::read_rds("all_mp.rds.gz")
# test_m_all
...
n_tp1_all <- test_data_1 %>% 
  distinct(ResearchID, Condition, Time, ot1, ot2, ot3) %>% 
  mutate(Trials = 30)
...
```{r}
pl <- augment_linpred(
  test_m_tp1, n_tp1_all, allow_new_levels = TRUE, 
  re_formula = NULL, nsamples = 500)

peaks <- pl %>% 
  group_by(ResearchID, Condition, .draw) %>% 
...
diffs <- peaks %>%
  ...
  group_by(ResearchID) %>% 
  summarise_at(vars(real, nonsense, diff), mean)
