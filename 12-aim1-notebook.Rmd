...
ggplot(intervals2) + 
  ...
  geom_segment(
    aes(y = parameter, yend = parameter, x = min(ll), xend = max(hh)), 
    color = "white", size = .25) +
...
```{r average-growth-curves, fig.cap = "", echo = FALSE, out.width = "80%", fig.height=4, fig.width=5}
lpred %>% 
  ...
  ggplot() + 
    ...
    geom_line(
      ...
      alpha = .1) +
    stat_summary(
      aes(y = Prop, group = Study), data = d_m, fun.y = "mean", geom = "line", 
      color = "grey50", size = 1) + 
    theme(
