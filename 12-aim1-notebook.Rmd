...
```{r post-pred, fig.cap = "(ref:post-pred)", out.width = "80%", fig.height=3, fig.width=6}
...
The model can simulated observations that looks like the observed data. 
...
The next
step is to ask how the model to simulate data for unobserved data---that is,
have the model simulate data for new unobserved children.
...
One of the predictions was that children would become less variable as they grew
older, as they converge on a mature level of performance. We can tackle this
question by simulating data from new participants and looking at the ranges of
predictions. The claim that children become less variable would imply that the
range of predictions should be narrower age 5 than for age 4 than age 3. Below
we plot the range of the predictions, both in terms of the full range (the
difference between minimum and maximum values) and in terms of the interquartile
range (the difference between the 75th percentile and the 25th percentile).
