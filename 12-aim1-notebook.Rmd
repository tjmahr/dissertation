...
Eyetracking data were analyzed using **Bayesian mixed
...
This amounted to study how the growth curve parameters changes year over
year in the study. We included dummy-coded indicators for Year 1, Year 2, and 
...
ggplot(correlations %>% filter(!is.na(var2))) + 
  ...
  labs(x = NULL, y = "Num. prior samples", 
       caption = "12,000 prior samples drawn per model")
```


```{r, include = FALSE}
gtools::rdirichlet(2000, c(1,1,4)) %>% colMeans()
gtools::rdirichlet(10, c(.1,.1,.1)) %>% colMeans()
gtools::rdirichlet(10, c(1,1,1)) %>% colMeans()
gtools::rdirichlet(2, c(400,400,400)) %>% colMeans()
gtools::rdirichlet(2, c(400,400,400))
qlogis(.70) - qlogis(.25)
qlogis(.65) - qlogis(.25)
pnorm(-1.96)
rnorm(10000) %>% quantile(c(.025, .975))
rnorm(10000) %>% quantile(c(.075, .975))
```
...
```{r pairwise-effects, fig.cap = "(ref:pairwise-effects)", echo = FALSE, out.width = "80%", fig.height=4, fig.width=5}
clean_names <- . %>% 
...
The average accuracy was `r pts$TP1` [90% UI: `r uis$TP1`] for timepoint 1,
`r pts$TP2` [`r uis$TP2`] for timepoint 2, and `r pts$TP3` [`r uis$TP3`] for
timepoint 3. The average accuracy increased by `r d_pts$TP2_TP1`
[`r d_uis$TP2_TP1`] from timepoint 1 to timepoint 2 and by `r d_pts$TP3_TP2`
[`r d_uis$TP3_TP2`] from timepoint 2 to timepoint 3. These results numerically
...
main_diffs$Study <- main_diffs$Study %>% 
  factor(c("TimePoint1", "TimePoint2", "TimePoint3", 
...
```{r, include = FALSE}
ot1_evt_tp3_uis <- main_diffs %>% 
  filter(coef == "ot1", variable == "r_TimePoint3_EVT_Standard") %>% 
  split(.$Study) %>% 
  get_uis()
# TODO: fix this correlation formatting

ot1_evt_tp3_pts <- main_diffs %>% 
  filter(coef == "ot1", variable == "r_TimePoint3_EVT_Standard") %>% 
  split(.$Study) %>% 
  get_pts() %>% 
  lapply(. %>% printy::fmt_leading_zero() %>% printy::fmt_minus_sign())

int_evt_tp3_uis <- main_diffs %>% 
  filter(coef == "intercept", variable == "r_TimePoint3_EVT_Standard") %>% 
  split(.$Study) %>% 
  get_uis()
# TODO: fix this correlation formatting

int_evt_tp3_pts <- main_diffs %>% 
  filter(coef == "intercept", variable == "r_TimePoint3_EVT_Standard") %>% 
  split(.$Study) %>% 
  get_pts() %>% 
  lapply(. %>% printy::fmt_leading_zero() %>% printy::fmt_minus_sign())

ot1_ppvt_tp2_uis <- main_diffs %>% 
  filter(coef == "ot1", variable == "r_TimePoint2_PPVT_Standard") %>% 
  split(.$Study) %>% 
  get_uis()
# TODO: fix this correlation formatting

ot1_ppvt_tp2_pts <- main_diffs %>% 
  filter(coef == "ot1", variable == "r_TimePoint2_PPVT_Standard") %>% 
  split(.$Study) %>% 
  get_pts() %>% 
  lapply(. %>% printy::fmt_leading_zero() %>% printy::fmt_minus_sign())

int_ppvt_tp2_uis <- main_diffs %>% 
  filter(coef == "intercept", variable == "r_TimePoint2_PPVT_Standard") %>% 
  split(.$Study) %>% 
  get_uis()
# TODO: fix this correlation formatting

int_ppvt_tp2_pts <- main_diffs %>% 
  filter(coef == "intercept", variable == "r_TimePoint2_PPVT_Standard") %>% 
  split(.$Study) %>% 
  get_pts() %>% 
  lapply(. %>% printy::fmt_leading_zero() %>% printy::fmt_minus_sign())
...
`r ot1_evt_tp3_pts$TimePoint1`, 90% UI [`r ot1_evt_tp3_uis$TimePoint1`], whereas
the concurrent lexical processing feature at year 3 only showed a correlation
of _r_ = `r ot1_evt_tp3_pts$TimePoint3`, [`r ot1_evt_tp3_uis$TimePoint3`],
...
[`r int_evt_tp3_uis[["TP1 - TP2"]]`] but much considerably greater than
...
the correlation for year 1 linear time, _r_\ = `r ot1_ppvt_tp2_pts$TimePoint1`,
...
main_diffs %>% 
  filter(variable == "r_TimePoint3_EVT_Standard") %>% 
    ...
    ggtitle("Correlations of curve features and expressive vocabulary at year 3") + 
...
main_diffs %>% 
  ...
  filter(Study %in% c("TimePoint1", "TimePoint2", "TP1 - TP2")) %>% 
    ...
    facet_wrap("coef") +
    labs(x = NULL, y = NULL, caption = "90% and 50% intervals") + 
    ggtitle("Correlations of curve features and receptive vocabulary at year 2") + 
    theme(title = element_text(size = rel(.9)))
```
