...
competitor. The semantic competitor exerts an advantage over the
...
which mutually activate them. The phonologically related words compete
...
other. The rhyme mismatches the input from the start of the word, so it
...
situation. At age 3, we observed 80% looking to the image of the shoes
...
for mispronunciation conditions and the real word condition. For
example, at age 5, *suze* reaches 60% looking to the familiar image and
*shoes* reaches 87% looking. Developmentally, children became more
likely to activate the target when given a mispronunciation, and this
change likely reflects the general improvements in activation
efficiency. Gains in activation efficiency are consistent with the
results familiar word recognition where children showed increases in
...
were all one-syllable words, so they did not have phonological substance
that could overlap with the target. If the mispronunciation-target pairs
were longer like a *beaker*--*speaker* rhyme, more segments would
...
the viability of the *geeve*-trolley pairing allows the child to select
...
low-frequency word. In both situations, the novel nonword is
...
visual referents occurred together, and each co-occurrence build up the
...
object. In contrast, for unambiguous nonword trials, children could
...
There are three open questions from this research. First, how does
lexical inhibition change over this developmental window? The results
from the first study show that phonologically and semantically similar
words become more relevant during word recognition as children grow
older. Thus, the words became more active, compared to the unrelated
word. I did not observe any clear changes in how quickly those words are
*rejected* as possible interpretations of the input, and thus, I could
not make any claims about the development of inhibition. In principle,
these developmental changes could have been observed in this experiment.
...
the advantage to decay more quickly at older ages. But the growth curves
observed here parallel; they decayed at the same rate. Changes in
lexical inhibition are detectable by an experiment like this, with a
target, competitor and an unrelated word, but in the present case, I did
not detect these changes. Thus, developmental changes in lexical
inhibition during the preschool years remains an open question.
...
favor *cat* and inhibit *cap* whereas a nonword splice cannot inhibit
...
the trajectory seems to be the development of connections so that the
phonological competitors can participate in word recognition with later
childhood being a time to develop inhibitory connections. In other
words, a child has to develop sensitivity to cohorts first in order to
demonstrate the ability to quickly inhibit them.
...
used a cross-splicing paradigm and observed that in adults, newly
...
[@McMurray2010]. These simulations shed light on their respective
...
Children's representations of familiar words continue to develop,
even if they ostensibly *know* the word. One might attribute this
development change to improvements in visual processing and sensory
...
representations can explain these results.
...
decreased with age. Thus, word recognition of familiar words is perhaps
best understood as a lexical measure that it needs to be scaled with
...
A final contribution comes from the second study. This study is limited
by how apparently easy it was for preschoolers. After all, it was
a 2-AFC task where one of the images is always a familiar object and the
other is an unfamiliar object. Those limitations were revealing, showing
that this design does not scale up for preschoolers developmentally.
Children mastered mutual-exclusivity-type referent selection on this
