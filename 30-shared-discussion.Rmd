...
first time they hear the word. (That is probably a dubious assumption
because when would a lexical item appear?) On these trials, however, the
children did know *sock* and that *geeve* was not the name for the sock,
so they looked to the trolley instead. For @McMurray2012, the problem
here is of reference selection: Children have to select a visual
referent for a word. In that model, all words can refer to all visual
referents initially and the model has to prune away unnecessary
connections to have selective word recognition. In this case, the strong
connection of the *sock*-sock pairing plus the viability of the
*geeve*-trolley pairing allow the child to select the correct referent
for the nonword. In TRACE simulations, @TRACE\_Mispro simulated this
situation by treating the nonword as a low-frequency word. In both
cases, the nonword is recognized because it is not affected by lexical
competition from any other relevant alternatives.
