...
description: > 
  This is a minimal example of using the bookdown package to write a book. The
...
multiple formats. This page is just a placeholder/scratch-paper so I have some
examples on-hand of how to use bookdown's syntax and features. 
...
The first one get an implicit label. The second one has an 
...
`r ''````{r table-single, tidy = FALSE, echo = FALSE}
knitr::kable(
...
```{r happy-cat, fig.cap = "(ref:happy-cat-cap)", out.width = "30%", fig.show = "hold"}
knitr::include_graphics(
  rep("./misc/happy-cat-grooming-itself-vector-file.png", 2)
)
```
