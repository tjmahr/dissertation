...
The lines `cbind(Primary, Others) ~ (ot1 + ot2 + ot3) * Study` fits a
cubic growth curve for each year of the study. It uses the R language's
formula convention to regress the looking counts onto an intercept term
...
correlate. For example, I would expect that participants with low
...
structure and point estimates for these effects were as follows:
...
LKJ(1) prior and the weakly informative LKJ(2) prior that I used. Under
LKJ(2), extreme correlations are less plausible. The prior shifts the
...
package [vers. 1.8.23; @Wood2017] with support from the tools in the
...
order to articulate the modeling decisions at play.

I first convert the categorical variables into the right types.
...
Then I can fit the generalized additive model. The outcome `elog` is the
empirical log-odds of looking to the phonological competitor relative to
the unrelated image.
