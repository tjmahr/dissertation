...
# readr::write_rds(test_m_tp1, "tp1_mp.rds.gz")
test_m_tp1 <- readr::read_rds("tp1_mp.rds.gz")
test_m_tp1
np <- nuts_params(test_m_tp1)
lp <- log_posterior(test_m_tp1)

bayesplot::mcmc_nuts_divergence(np, lp)
bayesplot::mcmc_nuts_stepsize(np, lp)
...
test_m_tp2
np <- nuts_params(test_m_tp2)
lp <- log_posterior(test_m_tp2)
...
test_m_tp3 <- readr::read_rds("tp3_mp.rds.gz")
test_m_tp3
np <- nuts_params(test_m_tp3)
lp <- log_posterior(test_m_tp3)
...
# test_m <- readr::read_rds("tp1_mp_2.rds.gz")
...
sss <- test_data$ResearchID %>% sample(6)
n <- test_data %>% 
...
ggplot(pl) + 
  ...
  geom_line(aes(group = interaction(.draw, Condition)), alpha = .05) +
  # stat_summary(aes(group = Condition), fun.data = median_hilow,
               # alpha = .1) +
  geom_line(aes(y = Target / Trials, group = Condition),
  ...
  labs(
    ...
    caption = "Sample of 6 of 149 total participants")

test_m
pp_check(test_m, nsamples = 50)
pl <- test_m %>% 
  posterior_linpred( 
    newdata = n, 
    allow_new_levels = TRUE, 
...
p[[3]] + 
  stat_smooth(
    aes(y = (Target / trials) * 21.5), 
    data = test_data, alpha = 1, se = FALSE, size = 2)


plot(marginal_effects(test_m, re_formula = NULL), ask = FALSE)
plot(marginal_effects(test_m, re_formula = NULL, method = "predict"), ask = FALSE)
