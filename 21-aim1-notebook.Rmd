...
ok <- m_e$data %>% 
  distinct(Time, Study, Condition, ot1, ot2, ot3) 

mat <- model.matrix(Time ~ (ot1 + ot2 + ot3) * Study * Condition, ok)
names(fixef(m_e)) == colnames(mat)


str(m_e$glmod)

m_e$glmod$reTrms$
ok$y <- as.vector(mat %*% fixef(m_e))

newdata <- m_e$data %>% 
  distinct(Time, Condition, Study, ot1, ot2, ot3) %>% 
  mutate(ResearchID = "NOPE")



ggplot(ok) + 
  aes(x = Time, y = y, color = Condition) + 
  geom_line() + 
  facet_wrap("Study")

d <- tristan::augment_posterior_linpred(m_e, newdata, nsamples = 100)
...
ggplot(diff_aug) + 
  aes(x = Time, y = elog , color = Condition, linetype = Condition) +
  geom_hline(color = "white", size = 2, yintercept = .5) +
  stat_summary() +
  stat_summary(aes(y = .posterior_value), fun.y = mean, geom = "line") +
  facet_wrap("Study")

means <- diff_aug %>% 
  group_by(.draw, Study, Condition, Time) %>%
  summarize_at(vars(.posterior_value:PropFitted), mean, na.rm = TRUE)

means %>% 
  filter(Condition == "real - nonsense") %>% 
  ggplot() + 
    aes(x = Time, y = PropFitted ) +
