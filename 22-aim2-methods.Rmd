...
p1 <- d_plot %>%
  ...
  ggplot() + 
    ...
    geom_line(
      ...
      alpha = .1, color = constants$col_off_black, 
      position = position_jitter(height = .015)) +
    stat_summary(
...
p2 <- d_plot %>%
  ...
  ggplot() + 
    ...
    stat_summary(
      ...
      fun.y = mean, geom = "line", size = 1.25) + 
    labs(x = constants$x_time,
         y = constants$y_prop_fam, caption = " ") + 
    guides(color = FALSE, linetype = FALSE)
p2
...
library(patchwork)
p3 <- p2 + 
  annotate("text", x = 340, y = .95, hjust = 0, color = "grey20",
           label = "trials starting on familiar image", size = 3.5) +
  annotate("text", x = 340, y = .05, hjust = 0, color = "grey20",
           label = "trials starting on unfamiliar image", size = 3.5)
p3 

cowplot::plot_grid(p3, p1, rel_widths = c(.4, .6), rel_heights = c(1, .8))

# {p3 + ggtitle("test")} + {p1 +     theme(strip.background.x = element_blank(), 
#           strip.text.x = element_blank())}

p3 + p1 + plot_layout(ncol = 2, widths = c(4, 6), height = c(4, 5))  + 
  plot_annotation(caption = "One line per child. Heavy line: Average of lines.")


p1 + p2 + plot_layout(ncol = 2, widths = c(6, 4))
p1 + p2 + plot_layout(ncol = 2, widths = c(6, 4)) + plot_annotation("Child hears real word")
