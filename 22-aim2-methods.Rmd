...
```{r}
htmlent::ents$`less-than or equal to`
```
;

Because this is a two image task, I took into account the location of
the child's gaze at the onset of the target noun. For each trial, I
counted the number of looks to the familiar object and the unfamiliar
object during the first 250 ms after target noun onset
(specifically, 0 ≤ *time* < 250 ms). If the majority of the looks
fell to the familiar object, then the trial was a familiar-initial
trial. An analogous rule labeled trials as unfamiliar-initial trials.
Ties were broken by favoring the early viewed image. For example, a tie
might be a trial with 7 frames of looking to the unfamiliar image,
...
image and it was excluded. Only 5% of trials were excluded in this way.
...
locaiton and time, and created orthogonal polynomials to use as time
...
d %>% 
  ...
  mutate(
    ...
    Condition = factor(
      ...
      levels = c("real", "nonsense", "MP"), 
      labels = c("Real word", "Nonword", "Mispronunciation"))) %>% 
  filter(Condition != "Mispronunciation", !is.na(Bias_Fam)) %>%
  ggplot() + 
    aes(x = Time, y = Prop) + 
    geom_hline(yintercept = .5, color = "white", size = 2) + 
    # geom_line(aes(group = interaction(Bias_Fam, ResearchID)), alpha = .16, color = constants$col_off_black) + 
    geom_line(aes(group = ResearchID), data = . %>% filter(Bias_Fam == "Familiar"), 
              alpha = .10, color = constants$col_off_black, position = position_jitter(height = .01)) +
    stat_summary(aes(color = Study, linetype = Bias_Fam), fun.y = mean, geom = "line", size = 1.25) +
    stat_summary(
      aes(color = Year, linetype = Bias_Fam), 
