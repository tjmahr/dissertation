...
the immediates peaks observed for the phonological foil.

In terms of lexical processing, this late timing is consistent with
cascading activation: Spoken words immediately activates phonological
neighborhoods with activation cascading to semantically related words.
As a particular word is favored, its semantic relatives receive more
activation. For example, children hear "find the shirt", activate the
target *shirt* but also activate other pieces of clothing like *dress*.
The late timing of looks to the semantic competitor reflects late,
secondary activation of the spoken word's semantic relatives. Put
another way, the activation of a semantic neighbor (like *dress*) is
greatest when the activation of the spoken word (*shirt*) is greatest
which happens once the competition among phonological alternatives has
resolved.

Under this explanation, children hear a word, activate it, and become
...
their initial interpretation, and children looking to the wrong image
because of confusion, lack of knowledge or overriding activation from
the semantic competitor. 

Initially, I had subscribed to the confusion or lack of knowledge interpretation. After all, these are young children and decisions like *bee* vs. *fly* or *goat* vs. *sheep* can be difficult. But there are two objections to that line of reasoning. First, we had piloted the set of words in preschool classrooms, so we confirmed that children will reliably and correctly point to *bee* even when a *fly* is an alternative. Second, we would a priori expect that children's confusion among words to be greatest when they are youngest and have much less experience with these semantic categories.
