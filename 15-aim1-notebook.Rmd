...
(ref:phon-vs-unre-fits) With each year of the study, children looked more to the
phonological foil, relative to unrelated image, during and after the target
noun. the target noun. Both figures show means for each year estimated by the
...
sem_tp2_p <- sem_smooths %>% 
  filter(term == "s(Time)") %>% 
  pull(p.value) %>% 
  format_p_value()

sem_tp2_edf <- sem_smooths %>% 
  filter(term == "s(Time)") %>% 
  pull(edf) %>% 
  printy::fmt_fix_digits(2)

sem_tp1_edf <- sem_smooths %>% 
  filter(term == "s(Time):S2TimePoint1") %>% 
  pull(edf) %>% 
  printy::fmt_fix_digits(2)

sem_tp1_p <- sem_smooths %>% 
  filter(term == "s(Time):S2TimePoint1") %>% 
  pull(p.value) %>% 
  format_p_value()

sem_tp3_edf <- sem_smooths %>% 
  filter(term == "s(Time):S2TimePoint3") %>% 
  pull(edf) %>% 
  printy::fmt_fix_digits(2)

sem_tp3_p <- sem_smooths %>% 
  filter(term == "s(Time):S2TimePoint3") %>% 
  pull(p.value) %>% 
  format_p_value()
```
...
750 ms and peak around 1300 ms. The peak of the curves increase with each year
of the study. The semantic foil shows a clear advantage over the unrelated image
...
semantic foil, relative to the unrelated image, with peak looking occuring after
