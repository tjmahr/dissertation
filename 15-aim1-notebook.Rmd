...
mgcv R package [vers. `r packageVersion("mgcv")`; @Wood2017] with
support from the tools in the itsadug R package
[vers. `r packageVersion("itsadug")`; @itsadug].[^1]

[^1]: Initially, I tried to use Bayesian polynomial growth curve
...
if (file.exists(phon_base)) {
  b2_age3 <- readr::read_rds(phon_base)
} else {
  b2_age3 <- bam(
    elog ~ s(Time) + s(Time, R, bs = "fs", m = 1, k = 5),
    data = phon_d_minp)
  readr::write_rds(b2_age3, phon_base)
}

if (file.exists(phon_minp)) {
  b2_age3_minp <- readr::read_rds(phon_minp)
} else {
  b2_age3_minp <- bam(
    elog ~ s(Time) + 
      s(ability) + 
    ...
    data = phon_d_minp)
  readr::write_rds(b2_age3_minp, phon_minp)
}

if (file.exists(phon_ppvt)) {
  b2_age3_ppvt <- readr::read_rds(phon_ppvt)
} else {
  b2_age3_ppvt <- bam(
    elog ~ s(Time) + 
      s(ppvt) + 
      ...
      s(Time, R, bs = "fs", m = 1, k = 5),
    data = phon_d_minp)
  readr::write_rds(b2_age3_ppvt, phon_ppvt)
}
...
```{r age3-phon-by-minp, fig.cap = "(ref:age3-phon-by-minp)"}
...
semy_age3_ppvt <- bam(
  elog ~ s(Time) + 
    ...
    s(Time, R, bs = "fs", m = 1, k = 5),
  data = semy_d_minp)
summary(semy_age3_ppvt)

semy_age3_ppvt_0 <- bam(
  elog ~ s(Time) + 
    ppvt + 
    s(Time, R, bs = "fs", m = 1, k = 5),
  data = semy_d_minp)
summary(semy_age3_ppvt_0)

semy_age3_minp_0 <- bam(
  elog ~ s(Time) + 
    ability + 
    s(Time, R, bs = "fs", m = 1, k = 5),
  data = semy_d_minp)
summary(semy_age3_minp_0)
```
