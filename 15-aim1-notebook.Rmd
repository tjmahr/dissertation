...
s_tp1_edf <- smooths %>% 
  filter(term == "s(Time):STimePoint1") %>% 
  pull(edf) %>% 
  printy::fmt_fix_digits(2)
...
s_tp2_edf <- smooths %>% 
  filter(term == "s(Time):STimePoint2") %>% 
  pull(edf) %>% 
  printy::fmt_fix_digits(2)
...
s_tp3_edf <- smooths %>% 
  filter(term == "s(Time):STimePoint3") %>% 
  pull(edf) %>% 
  printy::fmt_fix_digits(2)
...
freedom (EDF) = `r s_time_edf`, *p* `r s_time_p`, and for the age-3 smooth,
EDF = `r s_tp1_edf`, *p* `r s_tp1_p`. In contrast, the age-4 and age-5 smooths
did not differ significantly from the general time smooth, age-4 EDF =
`r s_tp2_edf`, age-4 *p* = `r s_tp2_p`, age-5 EDF = `r s_tp3_edf`, age-5 *p* =
`r s_tp3_p`. The shapes of the age-4 and age-5 growth curves showed the same general
