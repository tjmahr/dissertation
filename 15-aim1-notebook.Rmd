...
```{r, echo = FALSE, message = FALSE, out.width = "100%", fig.width = 6}
phon_d %>% 
...
To account for the sparseness of the data, we used the empirical log-odds (or empirical logit)
...
unrelated image has a (conventional) log-odds of log(4/1) = 1.39 and empirical
log-odds of log(4.5/1.5) = 1.10. This transformation fills in 0 values, and it
dampens the extremeness of some probabilities that happen in sparse count data.
...
```{r infobox-1-figs, include = FALSE}
library(mgcv)
...
`r tp3_prop` proportion units, *p* `r tp1_p`. Visually, this effect shows up in
