...
## Modeling looks to the phonological competitor over time
...
m <- glmer(cbind(Primary, Unrelated) ~ 
        Study * Focus * (ot1 + ot2 + ot3) + 
        (ot1 + ot2 + ot3 | ResearchID:Focus) + 
        (ot1 + ot2 + ot3 | Study:ResearchID:Focus), 
      family = binomial,
      data = d_m)
...
of the study. Each growth curve is the log-odds of fixating on a certain image
relative to the unrelated image. We use an indicator variable _focus_ to record
whether the image is the target or the phonological foil.
