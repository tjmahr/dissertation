...
After the naming task, children had a closed-set recognition task where
...
Children saw the two images of the two (say, trolley1.jpeg and
flasks1.jpeg), heard one of novel labels, and had to point to the named
object. In a later trial during testing, the other word was tested but
using different images with each object (trolley2.jpeg and
flasks2.jpeg). In a block of testing, there were 12 trials, 6 for
nonwords and 6 for mispronunciations.

Table TTT shows the results for each items.
...
mpn_items <- mp_norming %>% 
  group_by(WordGroup, MPNorm_Type, MPNorm_Item) %>% 
  summarise(
    NCorrect = sum(MPNorm_Correct),
    NTrials = n(),
    Percent = mean(MPNorm_Correct) %>% round(2) %>% scales::percent(),
    SE = se_prop(mean(MPNorm_Correct), NTrials) %>% round(3))


mpn_items_all <- mp_norming %>% 
  mutate(WordGroup = "(all)", MPNorm_Item = "&nbsp;") %>% 
  group_by(WordGroup, MPNorm_Type, MPNorm_Item) %>% 
  summarise(
    NCorrect = sum(MPNorm_Correct),
    NTrials = n(),
...
I performed an item-response analysis using a
mixed-effects logistic regression model. The model included varying
intercepts ("random intercept") for child, child x item type, item, and
item x word-group. The first two effects capture information about a
child's general ability and their ability on each type of item. The
second two effects capture information about an item's difficulty and
difficulty of object-pairs. I also asked whether growth curve peaks
predicted novel word recognition accuracy. For the nonwords, I used the
age-5 peak proportion of looks to the novel image for trials that
started on the familiar object. For the mispronunciations, I used the
peak looks to the familiar object on trials that started on the
unfamiliar object. 
...
by_cond_peaks <- unfam_peaks %>% 
  ...
  tidyr::separate(stat, c("MPNorm_Type", "stat")) %>% 
...
#   select(-real) %>% 
#   filter(Study == "Age 5") %>% 
#   tidyr::gather(MPNorm_Type, Peak, mispronunciation, nonword)

mp_norming_peaks <- by_cond_peaks %>% 
  inner_join(mp_norming) %>% 
  mutate(
    c_peak_mean = scale(peak_mean, scale = FALSE))
# priors <- c(
...
# m_norm2 <- brm(
#   MPNorm_Correct ~ 
#     MPNorm_Type * peak_mean + 
#     (1 | ResearchID/MPNorm_Type) + 
#     (1 | WordGroup) + 
#     (1 | MPNorm_Item),
#   prior = priors,
#   family = bernoulli,
...
# readr::write_rds(m_norm2, "./data/mp-norming-m2.rds.gz")

m_norm1 <- readr::read_rds("./data/mp-norming-m1.rds.gz")
m_norm2 <- readr::read_rds("./data/mp-norming-m2.rds.gz")
...
new_data <- mp_norming_peaks %>% 
  ...
  lapply(
    modelr::data_grid, 
    peak_mean = c(
      modelr::seq_range(peak_mean, 200), 
      mean(peak_mean, na.rm = TRUE)),
    ResearchID = "Average",
    MPNorm_Item = "ignore",
    WordGroup = "ignore",
    MPNorm_Type = unique(MPNorm_Type)) %>% 
  bind_rows()
...
means <- mp_norming_peaks %>% 
  group_by(MPNorm_Type) %>% 
  summarise(
    peak = mean(peak_mean, na.rm = TRUE),
    sd = sd(peak_mean, na.rm = TRUE),
    peak_upsd = peak + sd) %>% 
  split(.$MPNorm_Type)

norm_fixef <- posterior_samples(m_norm2, pars = "b_") %>% 
  as_tibble() %>% 
  transmute(
    b_mispronunciation = b_Intercept,
    b_nonword = b_Intercept + b_MPNorm_Typenonword,
        
    b_peak_mispronunciation = b_peak_mean,
    b_peak_nonword = b_peak_mean + `b_MPNorm_Typenonword:peak_mean`,
    ...
    d_mean_upsd_mispronunciation_prop =  b_mean_upsd_mispronunciation_prop - b_mean_mispronunciation_prop,
    d_mean_upsd_nonword_prop = b_mean_upsd_nonword_prop - b_mean_nonword_prop,
    
    or_nonword = exp(b_mean_nonword - b_mean_mispronunciation),
    or_peak_mispronunciation_10 = exp(b_peak_mispronunciation * .1),
    or_peak_mispronunciation_10 = exp(b_peak_mispronunciation * .1)) 
...
# sjPlot::tab_model(m_norm2)

mean_mp_peak <- means$mispronunciation$peak %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

mean_nw_peak <- means$nonword$peak %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

sd_mp_peak <- means$mispronunciation$sd %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

sd_nw_peak <- means$nonword$sd %>% 
  printy::fmt_fix_digits(3) %>% 
  printy::fmt_leading_zero()
```

For a child with an average misproncunciation peak of `r mean_mp_peak`,
the predicted proportion correct on the mispronunciation retention
trials was `r props$b_mean_mispronunciation_prop`. For a child with an
average peak on the nonword trials of `r props$b_mean_mispronunciation_prop`,
the predicted proportion correct on the mispronunciation retention
trials was `r props$b_mean_nonword_prop`. Children were much more
successful on the nonword trials. A 1-SD, or `r sd_mp_peak`, increase in
...
to the familiar word on these mispronunciation trials were less
successfull during the retention trials. A 1-SD increase, or
`r sd_nw_peak` to 1.00, in nonword peaks predicted a change in
proporiton correct of `r props_sd$d_mean_upsd_nonword_prop`. The
uncertainty interval here includes positive and negative values so it is
uncertain where the effect is positive or negative, so I conclude that
there is no effect in the nonword case.

Add n.

Figure \@ref(fig:mp-norm-trials-by-peak) visualizes this effect.

(ref:mp-norm-trials-by-peak) Effect of growth curve peaks on children's accuracy on retention trials. For the mispronunciations, I used the peak looks to the familiar image on trials where the child started on the unfamilar image, so it represents, say, how much a child looked at *shoes* given "suze". More permissive listeners looked did more poorly on the retention trials. For the nonwords, I used the peak look to the unfamilar image on trials where the child started on the familiar image.
...
```{r mp-norm-trials-by-peak, fig.cap = "(ref:mp-norm-trials-by-peak)", fig.width=5, fig.height=3, out.width="50%"}
aaa %>% 
  group_by(MPNorm_Type, .draw) %>% 
  ggplot() + 
    aes(x = peak_mean, y = .posterior_value, color = MPNorm_Type) + 
  geom_hline(yintercept = .5, size = 2, color = "white") +
      stat_summary(
      aes(y = MPNorm_Correct),
      data = mp_norming_peaks,
      fun.data = mean_se_prop,
      geom = "point",
      position = position_jitter(.01, .01), shape = 1)  + 
    stat_summary(
      aes(fill = MPNorm_Type),
      fun.data = median_hilow,
    ...
    geom_text(
      ...
      data = data_frame(
        ...
        MPNorm_Type = c("mispronunciation", "nonword"),
...
min_peak <- condition_means %>% 
  filter(Condition == "mp", Study == "Age 3") %>% 
...
ggplot(condition_means) + 
  ...
  geom_text(
    ...
    family = "Lato Semibold") +
  geom_point(
    data = . %>% anti_join(min_peak),
    position = position_jitterdodge(jitter.width = 0.25, dodge.width = 1.05), 
    alpha = .5, 
    shape = 1) +
  ggrepel::geom_text_repel(
    aes(label = "posterior mean"),
    force = 10,
    xlim = c(.90, NA),
    # ylim = c(NA, .07),
    color = "grey30",
...
```{r, out.width = "100%"}
knitr::include_graphics("./misc/placeholder.png")
```
...
Child level predictors only were preditive at age 3 for curve valley. In
...
In these analyses, I defined a weighted quadratic growth curve feature
