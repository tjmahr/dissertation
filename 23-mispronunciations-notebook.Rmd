...
# mcmc_intervals(data.frame(x = test$b_Intercept_Prop_Age3))
# mcmc_intervals(test,  regex_pars = "Intercept_Prop")
# mcmc_intervals_data(test,  regex_pars = "Intercept_Prop")
...
time1$per_d_ot1_Age4 <- time1$per_d_ot1_Age4 %>% 
  stringr::str_replace_all("([.]\\d+)", . %>% as.numeric %>% scales::percent())
time1$per_d_ot1_Age5 <- time1$per_d_ot1_Age5 %>% 
  stringr::str_replace_all("([.]\\d+)", . %>% as.numeric %>% scales::percent())
...
familiar image, was `r props$b_Age3`. In this situation, the term is
...
showing a change of `r time1$per_d_ot1_Age4` from age 3 to age 4, and a
change of `r time1$per_d_ot1_Age5` from age 4 to age 5. 
