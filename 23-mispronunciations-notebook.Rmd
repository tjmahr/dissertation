...
(ref:unfam-initial-mp-trials) Average looks to familiar image for mispronunciation trials starting on the unfamiliar image at each age. Lines represent 100 posterior predictions of the group average (the average of participants' individually predicted growth curves). 
...
```{r plot-evt-mp-unfam-peaks, fig.cap = "(ref:plot-evt-mp-unfam-peaks)", fig.scap = "(ref:plot-evt-mp-unfam-peaks-scap)", out.width = out_tex80_else50, fig.width = 4.25, fig.height = 4}
p_evt_peaks
```
...
```{r mp-fam-gca-features, fig.cap = "(ref:mp-fam-gca-features)", fig.scap = "(ref:mp-fam-gca-features-scap)", eval = TRUE, fig.width = 7, fig.height = 3, out.width = "100%"}
draws <- mp_fam_fixef %>% 
...
(ref:age3-valley-curves-scap) Growth curves for mispronunciation trials starting on the familiar object at age 3 with children binned by growth curve valley.
...
mp_norming <- readr::read_csv("./data-raw/age5-mp-norming.csv") %>% 
  ...
  mutate(
    Study = convert_study_to_age(Study),
    Item = ifelse(Item == "shan", "shann", Item),
    Item = ifelse(Item == "sues", "suze", Item))
...
if (knitr::is_latex_output()) {
  to_print <- both %>% 
    ...
    mutate(
      `Percent Correct` = `Percent Correct` %>% 
        knitr:::escape_latex() %>% 
        stringr::str_replace("\\\\&plusmn;", "$\\\\pm$")) %>% 
    rename(`Percent Correct $\\pm$ SE` = `Percent Correct`) %>% 
    knitr::kable(
      format = "latex",
      align = "lllrr", 
      booktabs = TRUE, 
  ...
  to_print <- both %>% 
    rename(`Percent Correct &plusmn; SE` = `Percent Correct`) %>% 
    knitr::kable(
      align = "lllrr", 
      caption = "Results for the item retention tests.")
}
...
model included data from `r n_peaks` children.
