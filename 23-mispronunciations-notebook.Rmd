...
a mispronunciation, they on average shift their looks to the familiar
...
Given the interplay of these features, I will avoid assigning a developmental intepretation to individual features. The two main developmental features here are that the age 3 curve starts to fall a little later than the other curves. (This difference is more apparent in Figure \@ref(fig:fam-initial-mp-trials).)

The individual feature changes do not have a developmental interpretation. It would be incorrect to say that the younger children show a 
story, if any, is that children stick with the
...
`r r2_valley_minp`, *n* = `r nobs(m_fam_valley_prop_age3)`. An
increase in minimal pair discrimination accuracy of .1 probability units
predicts a decrease of growth curve peak by `r neg_valley_delta_by_minp1`.
...
pair accuracy is `r minp_cutpoint` or greater (at the
`r minp_cutpoint_tile`-percentile or greater). Conversely, the simple
effect of minimal pair accuracy is no longer significant when expressive
vocabulary standard score is `r evt_cutpoint` or greater (at the
`r minp_cutpoint_tile`-percentile or greater). **interpret this effect**
