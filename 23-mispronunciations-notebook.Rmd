...
peak of `r delta_by_15`, *R*^2^ = `r peak_evt_r_squared`---see
...
the familiar image, was `r props$b_Age3`. In this situation, the term is
...
age 4, the average probability decreased by `r props$neg_d_Age4` to
`r props$b_Age4`, and at age 5 the average probability returns to age-3
levels `r props$b_Age5`. These effects contribute to how the age 4
...
by `r time1$neg_per_d_ot1_Age4` from age 3 to age 4 and decreasing by
`r time1$neg_per_d_ot1_Age5` from age 4 to age 5.
...
age 5, the quadratic is `r time2$neg_per_d_ot2_Age5` smaller than at
...
peak_groups <- fam_peaks %>% 
  ...
  mutate(
    ...
    num = ntile %>% fct_add_counts(first_fmt = "{levels} ({counts} children)") %>% as.character()) %>% 
...
ggplot(to_plot) + 
  ...
  labs(
    x = constants$x_time,
    y = constants$y_prop_fam,
    title = "Age 3 children binned into sextiles by valley",
...
peak_groups %>% 
  ungroup() %>% 
  inner_join(scores) %>% 
  lm(Peak ~ scale(EVT_Standard) * Study, .) %>% 
  summary()
...
f_m_evt_age3 <- peak_groups %>% 
  left_join(scores) %>% 
  filter(Study == "Age 3") %>% 
  lm(Peak ~ scale(EVT_Standard, scale = FALSE), .)
summary(f_m_evt_age3)

# Small effect
neg_f_delta_by_15 <- (coef(f_m_evt_age3)[2] * 15) %>% 
  abs() %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

f_peak_evt_r_squared <- f_m_evt_age3 %>% 
  broom::glance() %>% 
  pull(r.squared) %>%
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()
...
f_m_minp_age3_simple <- peak_groups %>% 
  left_join(scores) %>% 
  filter(Study == "Age 3") %>% 
  lm(Peak ~ scale(MinPair_ProportionCorrect, scale = FALSE), .)
summary(f_m_minp_age3_simple)

# Small effect
neg_f_delta_minp_by_1 <- (coef(f_m_minp_age3_simple)[2] * .1) %>% 
  abs() %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

f_peak_minp_r_squared <- f_m_minp_age3_simple %>% 
  broom::glance() %>% 
  pull(r.squared) %>%
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()
...
f_m_evt_minp_age3 <- 
  lm(
    ...
    evt_minp_age3)
summary(f_m_evt_minp_age3)

raw_m_evt_minp_age3 <- lm(
  Peak ~ MinPair_ProportionCorrect * EVT_Standard, 
  evt_minp_age3)
summary(raw_m_evt_minp_age3)
...
minp_cutpoint <- raw_m_evt_minp_age3 %>% 
...
evt_cutpoint <- raw_m_evt_minp_age3 %>% 
  jtools::johnson_neyman(
    pred = MinPair_ProportionCorrect, 
    modx = EVT_Standard) %>% 
  getElement("bounds") %>% 
...
# minp_set <- peak_groups %>% 
#   left_join(scores) %>% 
#   filter(Study == "Age 3") 
# 
# m_minp_set <- minp_set %>% 
#   lm(Peak ~ MinPair_ProportionCorrect * EVT_Standard, .)
# 
# minp_set %>% 
#   mutate(minp = ntile(MinPair_ProportionCorrect, 3)) %>% 
#   filter(!is.na(minp)) %>% 
#   ggplot() + 
#   aes(x = EVT_Standard, y = Peak, color = factor(minp)) + 
#   geom_point() + 
#   stat_smooth(method = "lm")
# 
# minp_set %>% 
...
#   filter(!is.na(evt)) %>% 
#   ggplot() + 
#   aes(x = MinPair_ProportionCorrect, y = Peak, color = factor(evt)) + 
#   geom_point() + 
...
evt_minp_neg_f_delta_by_1_mip <- (coef(f_m_evt_minp_age3)[2] * .1) %>% 
  abs() %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

evt_minp_f_delta_by_15_evt <- (coef(f_m_evt_minp_age3)[3] * 15) %>% 
  abs() %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

evt_minp_f_peak_r_squared <- f_m_evt_minp_age3 %>% 
  broom::glance() %>% 
  pull(r.squared) %>%
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()
...
`r f_peak_evt_r_squared`, *n* = `r nobs(f_m_evt_age3)`. A 15-point
increase in expressive vocabulary predicted decrease in growth curve
valley of `r neg_f_delta_by_15`. At the other ages, the effects are
...
`r f_peak_minp_r_squared`, *n* = `r nobs(f_m_minp_age3_simple)`. An
increase in minimal pair discrimination accuracy of .1 probability units
predicts a decrease of growth curve peak by `r neg_f_delta_minp_by_1`.

I regressed age-3 valleys onto both EVT-2 standard score, minimal-pair
discrimination accuracy and their interaction, and all effects were
statistically significant, *R*^2^ =
`r evt_minp_f_peak_r_squared`, *n* = `r nobs(f_m_evt_minp_age3)`. The effects of vocabulary and minimal-pair discrimination were both negative, while their interaction was positive For an average participant, a 15-point increase in expressive vocabulary predicted decrease in growth curve valley of `r evt_minp_f_delta_by_15_evt`, and an increase minimal pair accuracy of .1 predicted a decrease in valley of `r evt_minp_f_delta_by_15_evt`. The positive interaction term means that increasing one of the predictors simulataneously weakens the effect of the other. Thus, the effect of expressive vocabulary is no longer nominally significant when minimal pair accuracy is `r minp_cutpoint` or greater, and conversely the minimal pair accuracy effect is no longer significant when expressive vocabulary is `r evt_cutpoint` or greater. (The average accurary is.... and vocabulary is ... .)
...
```{r}
# Minimal pairs predicting Age 3 minimum
peak_groups %>% 
  ungroup() %>% 
  inner_join(scores) %>% 
  filter(Study == "Age 3") %>% 
  ggplot() + 
    aes(x = MinPair_ProportionCorrect, y = Peak, color = Study) + 
    geom_point() + 
    stat_smooth(method = "lm")
...
peak_groups %>% 
  ...
  inner_join(scores) %>% 
  filter(Study == "Age 3") %>% 
  lm(Peak ~ MinPair_ProportionCorrect, .) %>% 
  summary()
...
```{r}
# Minimal pairs predicting lowest value of the curve at age 3
...
m_f_new_fits_each %>% 
  ...
  ggplot() + 
    ...
    labs(
      x = constants$x_time,
      y = constants$y_prop_fam,
      title = "Age 3 children binned into sextiles by rise after min value",
      caption = "10 posterior samples per child")
```

```{r}
# Nothing predicts diffs
...
#   filter(Study == "Age 3") %>% 
#   ggplot() + 
#     aes(x = MinPair_ProportionCorrect, y = Diff, color = Study) + 
#     geom_point() + 
#     stat_smooth(method = "lm")
# 
# with_peaks %>% 
...
#   inner_join(scores) %>% 
#   filter(Study == "Age 3") %>% 
#   lm(Diff ~ MinPair_ProportionCorrect, .) %>% 
#   summary()
# 
# with_peaks %>% 
#   ungroup() %>% 
#   inner_join(scores) %>% 
#   filter(Study == "Age 3") %>% 
#   lm(Diff ~ MinPair_ProportionCorrect * EVT_Standard, .) %>% 
#   summary()
```
...
m_f_new_fits_each %>% 
  ...
  ggplot() + 
    ...
    guides(color = FALSE) + 
    labs(
      title = "Age 3 children binned into sextiles by differences",
...
#   inner_join(scores) %>% 
#   ggplot() + 
#   aes(x = EVT_Standard, y = Delta, color = Study) + 
#   geom_point() + 
#   stat_smooth(method = "lm")
...
#   filter(Study == "Age 3") %>%
#   ggplot() +
#     aes(x = MinPair_ProportionCorrect, y = Delta, color = Study) +
#     geom_point() +
#     stat_smooth(method = "lm")
# 
# deltas %>%
...
#   inner_join(scores) %>%
#   filter(Study == "Age 3") %>%
#   lm(Delta ~ MinPair_ProportionCorrect, .) %>%
#   summary()
# 
# deltas %>%
#   ungroup() %>%
#   inner_join(scores) %>%
#   filter(Study == "Age 3") %>%
#   lm(Delta ~ MinPair_ProportionCorrect * EVT_Standard, .) %>%
#   summary()
...
#   inner_join(scores) %>% 
#   ggplot() + 
#   aes(x = Delta, y = EVT_Standard, color = Study) + 
#   geom_point() + 
#   stat_smooth(method = "lm")
```
...
m_f_new_fits_each %>% 
  ...
  ggplot() + 
    ...
    guides(color = FALSE) + 
    labs(
      title = "Age 3 children binned into sextiles by quadratic trend",
      caption = "10 posterior samples per child")
...
bowls %>% 
  ...
  inner_join(scores) %>%
  ggplot() +
  aes(x = EVT_Standard, y = estimate, color = Study) +
  geom_point() +
  stat_smooth(method = "lm")
...
bowls %>% 
  ...
  filter(Study == "Age 3") %>%
  ggplot() +
    aes(x = MinPair_ProportionCorrect, y = estimate, color = Study) +
    geom_point() +
    stat_smooth(method = "lm")
...
bowls %>% 
  ...
  inner_join(scores) %>%
  filter(Study == "Age 3") %>%
  lm(estimate ~ MinPair_ProportionCorrect, .) %>%
  summary()
...
bowls %>% 
  ...
  inner_join(scores) %>%
  filter(Study == "Age 3") %>%
  lm(estimate ~ MinPair_ProportionCorrect + EVT_Standard, .) %>%
  summary()
