...
minp_neg_f_delta_by_1 <- (coef(f_m_evt_minp_age3)[2] * .1) %>% 
  abs() %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

evt_neg_f_delta_by_15 <- (coef(f_m_evt_minp_age3)[3] * 15) %>% 
  abs() %>% 
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()

minp_f_peak_evt_r_squared <- f_m_evt_minp_age3 %>% 
  broom::glance() %>% 
  pull(r.squared) %>%
  printy::fmt_fix_digits(2) %>% 
  printy::fmt_leading_zero()
...
I regressed age-3 valleys onto both EVT-2 standard score and minimal 
