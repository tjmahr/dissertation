...
```{r aim2-real-nonword-means, out.width = "80%", fig.height = 5, fig.width = 6}
theme_set(theme_teej())
# m_tp1
# m_tp2
# m_tp3
...
```{r aim2-gca-peaks, out.width = "50%", fig.height = 3, fig.width = 4}
ant <- data_frame(
  diff = -.6, 
  Study = "Age 3", 
  ...
  mutate(Condition = convert_condition_to_name(Condition)) %>% 
    ggplot() + 
      ...
      geom_text(
        ...
        family = "Lato Medium") +
      geom_point(
        position = position_jitterdodge(jitter.width = 0.2, dodge.width = 1.05), 
        alpha = .5, shape = 1) +
      guides(color = FALSE) +
      geom_boxplot(
        width = .2, 
    ...
    labs(
      ...
      x = NULL,
      y = NULL,
      caption = "Open points: Participant posterior means.") + 
    scale_y_continuous(
...
The growth curve peaks follow a similar pattern of results. 

Figure X
shows the distribution of the posterior means of the growth curves for
condition. Here, descriptive statistics reveal a great deal about developmental trends for this task.

At age 3, the median peak values are similar around 80%. The peaks
increase for the nonword condition in the following year, with a median
value of .... It is worth emphasizing what this median tells us: At
age 4, half of the children had a peak looking probability of xxx. In
other words, children are performing at the ceiling on this task by
age 4. To quantify the degree of ceiling performance, I calculcated the
number of children per condition with a growth curve peak greater than
or equal to .99 over the posterior distribution. For the nonword
condition, there were `r nons_ceilings$TP1` children who performed at
ceiling at age 3, `r nons_ceilings$TP2` at age 4, `r nons_ceilings$TP3`
and at age 5. For the real word condition, the number of children
attaining ceiling performance was more uneven: there were
`r real_ceilings$TP1` ceiling performers at age 3,
`r real_ceilings$TP2`, and `r real_ceilings$TP2` at age 5.
...
To compare peaks between studies more formally, I fit a linear mixed
effects model---with restricted maximum likelihood--i.e., using
lme4---to regress growth curve peak onto age group and experimental
condition and age x condition with randomly varying intercepts for child
and child-year. For these comparisons, I declare significance when the
t-value statistic for a model fixed effect has a magnitude of 2 or
greater. At age 3, the two condition did not significantly differ (t =
). At age 4, nonword peaks were xx greater than the real word peaks (t =
) and at age 5, the nonword peaks were xxx greater than the real word
peaks (t = ). For the nonword condition there was a significant increase
in the peaks from age 3 to age 4 (change = , t = ) whereas there was no
improvement from age 4 to age 5 (change =, t = ). In the real word
condition, there was only significant improvement from age 4 to age 5.
(change = , t = ).

In short, there is a decisive advantage for the nonword condition after
age 3. Further, performance begins to saturate at age 4 with the group
averages above 90% peak looking probabilities. The real word condition
is more anomalous with performance only increasing from age 4 to age 5.
...
#   group_by(Study, .draw) %>%
#   summarise(
#     non_50 = median(nonsense),
#     non_05 = quantile(nonsense, .05),
#     non_95 = quantile(nonsense, .95))

do_it_1 %>% filter(nonsense >= .99) %>% count(Study)
...
do_it_1 %>% 
  group_by(Study) %>% 
  summarise(med_non = median(nonsense))
...
To evaluate the growth curve peaks, 
...
```{r aim2-gca-diffs, out.width = "50%", fig.height = 3, fig.width = 4}
theme_set(theme_teej())
...
```{r}
# too_check <- peaks %>% filter(real < .2) %>% distinct(Study, ResearchID)
# 
# d %>% 
#   inner_join(too_check) %>% 
#   ggplot() + 
#   aes(x = Time, y = Prop, color = Condition, linetype = Bias_Fam) + 
#   geom_line() + 
