...
```{r, age-5-from-peaks, out.width = "50%", fig.height = 3, fig.width = 3}
...
# mod_robust <- brm(bf(EVT_Standard ~ diff),
#                   family=student,
#                   data = testy, cores=4)
# 
# mod_robust
# 
# summary(lm(EVT_Standard ~ diff, data = testy))
# plot(marginal_effects(mod_robust, spaghetti = TRUE, nsamples = 100), points = TRUE)
# 
# do_it_1 %>% 
#   filter(Study == "TimePoint1") %>% 
#   select(-Study) %>% 
...
#   labs(
#     x = "Nonword advantage at age 3", 
#     y = "EVT-2 standard score at age 5")
# 
# do_it_1 %>% 
#   filter(Study == "TimePoint1") %>% 
#   select(-Study) %>% 
...
#   inner_join(tp3) %>% 
#   ggplot() + 
#     aes(x = nonsense, y = EVT_Standard) + 
#     geom_point() +
#     stat_smooth(method = "lm") + 
#   labs(
#     x = "Nonword peak at age 3", 
#     y = "EVT-2 standard score at age 5")
# 
...
w_evt %>%
  modelr::add_residuals(
    model = lm(TimePoint3 ~ TimePoint1, .),
    var = "EVT_Residualized") %>%
  ggplot() +
    ...
    labs(
      x = "Nonword peak at age 3", 
      y = "Residualized age-5 EVT-2 score")
...
```{r}
summary(lm(scale(TimePoint3) ~ scale(TimePoint1), w_evt))
