...
```{r aim2-gca-peaks, out.width = "50%", fig.height = 2.5, fig.width = 4}
ant <- data_frame(
  diff = -.6, 
  Study = "Age 3", 
...
nonsense_change <- {coef(evt_t1_nonsense)[3] / 10} %>% 
  printy::fmt_fix_digits(1)

z_nonsense_change <- {coef(evt_t1_z_nonsense)[3] / 10} %>% 
  printy::fmt_fix_digits(2)
...
nonsense_delta <- nonsense_delta %>% 
  ...
  printy::fmt_leading_zero()
# 
# real_delta <- adj_rsquare(evt_t1_z_real) - adj_rsquare(evt_t1)
...
```{r, age-5-from-peaks, out.width = "100%", fig.height = 2.5, fig.width = 6}
p1 <- w_evt %>%
  ggplot() +
    aes(x = nonsense, y = EVT_Residualized) +
    geom_point() +
...
library(patchwork)
p1 + p2 + p3 + plot_layout(nrow = 1) + plot_annotation(caption = "Age 3 predictors")
```
