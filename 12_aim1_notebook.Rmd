...
```{r pairwise-effects, out.width = "80%", fig.height=4, fig.width=5}
clean_names <- . %>% 
...
```{r intercept-differences, out.width = "60%", fig.height=2.5, fig.width=4}
proportions <- b %>% 
  as.data.frame() %>%
  transmute(
    `TP1` = `(Intercept)`,
...
```{r, eval = FALSE}
scores <- readr::read_csv("./data-raw/scores_for_participant_summary.csv") %>% 
  select(Study, ResearchID, Age, EVT_Standard, PPVT_Standard)

ppvt_tp2_changes <- scores %>% 
  select(-EVT_Standard, -Age) %>% 
  tidyr::spread(Study, PPVT_Standard) %>% 
  mutate(PPVT_Std_Growth_TP2 =  TimePoint2 - TimePoint1) %>% 
  select(ResearchID, PPVT_Std_Growth_TP2) %>% 
  filter(!is.na(PPVT_Std_Growth_TP2))
...
ggplot(with_prop_changes) + 
  aes(x = TP2_Change_PPVT_Standard, y = TP2_Change_Prop) + 
  geom_point() + 
  stat_smooth(method = "lm")

ggplot(with_prop_changes) + 
  aes(x = TP2_Change_EVT_Standard, y = TP2_Change_Prop) + 
  geom_point() + 
  stat_smooth(method = "lm") 

ggplot(with_prop_changes) + 
  aes(x = TP3_Change_EVT_Standard, y = TP3_Change_Prop) + 
  geom_point() + 
  stat_smooth(method = "lm") 

ggplot(with_prop_changes) + 
  aes(x = TP2_Change_Age, y = TP2_Change_Prop) + 
  geom_point() + 
  stat_smooth(method = "lm") 

ggplot(with_prop_changes) + 
  aes(x = TP3_Change_Age, y = TP3_Change_Prop) + 
  geom_point() + 
  stat_smooth(method = "lm") 
