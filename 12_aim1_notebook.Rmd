...
ggplot(lpred) + 
  ...
  guides(color = guide_legend(title = NULL, override.aes = list(alpha = 1))) +
  labs(
    title = "Observed data and 100 posterior simulations",
    x = "Time after target onset",
...
```{r}
intervals <- as.data.frame(b) %>% 
  mcmc_intervals_data(regex_pars = "Sigma")

intervals$group <- intervals$parameter %>% 
  stringr::str_detect("Sigma\\[Study:ResearchID") %>% 
  ifelse(., "Child", "Child-Study")

terms <- intervals$parameter %>% 
  stringr::str_extract("ResearchID.+") %>% 
  stringr::str_replace("ResearchID:", "") %>% 
  stringr::str_replace_all("[()]|\\]", "") %>% 
  stringr::str_split(",", n = 2)

intervals$term1 <- terms %>% lapply(getElement, 1) %>% unlist()
intervals$term2 <- terms %>% lapply(getElement, 2) %>% unlist()

facet_data <- intervals 
facet_data$parameter <-
  facet_data$parameter %>%
  str_replace("\\[.+ID:", "[") %>% 
  str_replace_all("[()]", "") %>%
...
# Wait these are not scales/correlations!!
facet_data$class <- ifelse(facet_data$term1 == facet_data$term2, 
                           "scale", "correlation")

ggplot(facet_data %>% filter(class == "scale")) + 
  aes(y = parameter) +
  geom_vline(xintercept = 0, size = 2, color = "white") +
  ...
  scale_y_discrete(labels = parse_text) +
  labs(x = NULL, y = NULL, caption = "90% and 50% intervals") + 
  ggtitle("Random effect scales") + 
  facet_wrap("group", ncol = 1, scales = "free_y", strip.position = "left") + 
  theme(strip.placement = "outside", 
        strip.background = element_rect(fill = NA)) 
        # strip.text.y = element_text(angle = 180, hjust = 1, vjust = 1, 
        #                             margin = margin(r = -50)))
```

```{r}
ggplot(facet_data %>% filter(class == "correlation")) + 
  aes(y = parameter) +
  geom_vline(xintercept = 0, size = 2, color = "white") +
  # Draw medians with + then draw white horizontal lines over the horizontal 
  # parts of the + symbols
  ...
  scale_y_discrete(labels = parse_text) +
  labs(x = NULL, y = NULL, caption = "90% and 50% intervals") + 
  ggtitle("Random effect correlations") + 
  facet_wrap("group", ncol = 1, scales = "free_y", strip.position = "left") + 
  theme(strip.placement = "outside", 
        strip.background = element_rect(fill = NA)) 
```
...
Focus on the area under the curve and linear time components.

```{r}
facet_data %>% 
  filter(term1 %in% c("Intercept", "ot1")) %>% 
  ggplot() + 
  aes(y = parameter) +
  geom_vline(xintercept = 0, size = 2, color = "white") +
  # Draw medians with + then draw white horizontal lines over the horizontal 
  ...
  scale_y_discrete(labels = parse_text) +
  labs(x = NULL, y = NULL, caption = "90% and 50% intervals") + 
  ggtitle("Random effect scales and correlations") + 
