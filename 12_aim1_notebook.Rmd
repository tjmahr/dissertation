...
#   tibble::rowid_to_column(".observation")
# 
# d_pred <- posterior_predict(b, draws = 100) %>% 
#   reshape2::melt(
#     varnames = c(".draw", ".observation"),
#     value.name = ".posterior_value") %>% 
#   left_join(d_m) %>% 
...
#   summarise(acc = sum(Primary) / sum(Primary + Others)) %>% 
#   ungroup()
# 
# 
# acc_ranks <- acc_ranks %>% 
...
# 
# ranks <- acc_ranks %>% 
#   tidyr::spread(Study, acc) %>% 
#   mutate_at(c("TimePoint1", "TimePoint2", "TimePoint3"), ~ row_number(desc(.)))
# 
# 
# cor(ranks[-1], use = "pairwise.complete.obs", method = "kendall") %>% round(2)
# cor(ranks[-1], use = "pairwise.complete.obs", method = "spearman") %>% round(2)
# 
# ggplot(ranks %>% tidyr::gather(Study, Rank, -ResearchID)) + 
#   aes(x = Study, y = Rank) + 
