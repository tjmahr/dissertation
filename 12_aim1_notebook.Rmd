...
We predicted that children would show stable longitudinal differences such that
...
mean_rank <- function(xs) {
  glue::glue("{printy::fmt_minus_sign(printy::fmt_fix_digits(xs, 2))} ({zero_pad_int(rank(-xs))})")
}
reduced_data %>% 
  filter(coef == "intercept") %>% 
  ...
  mutate(
    TimePoint1 = mean_rank(TimePoint1), 
    TimePoint2 = mean_rank(TimePoint2), 
    TimePoint3 = mean_rank(TimePoint3)) %>% 
  rename(
...
_W_'s for these growth curve features.

(ref:kendall-stats) Uncertainty intervals for the Kendall's coefficient of
concordance. Random ratings provide a baseline of null _W_
statistics. The intercept and linear time features are decisively
non-null, indicating a significant degree of correspondence in children's
relative word recognition reliability and efficiency over three years of study.
...
posterior_w %>% 
  ...
  ggplot() + 
    ...
    labs(
      ...
      caption = "Posterior median with 90% and 50% intervals",
      subtitle = "Kendall's W. Raters: 3 time points. Items: 123 children.")
```
...
individual differences in lexical processing are stable over time.
