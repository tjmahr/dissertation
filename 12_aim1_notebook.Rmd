...
> As a consequence, individual differences in word recognition at age 3, for example, will be more discriminating and predictive of age 5 language outcomes than differences at age 4.
...
test <- fits %>% 
  ...
  summarise(c1 = cor(.posterior_value, TimePoint3_EVT_Standard, use = "pairwise.complete"))
...
widely %>% tidy_corr(ends_with("EVT_GSV"))


widely %>% 
  tidy_corr(TimePoint3_EVT_GSV, ends_with("intercept"), ends_with("ot1")) %>% 
  filter(column1 == "TimePoint3_EVT_GSV")

widely %>% 
  tidy_corr(TimePoint2_PPVT_GSV, TimePoint1_ot1, TimePoint2_ot1) %>% 
